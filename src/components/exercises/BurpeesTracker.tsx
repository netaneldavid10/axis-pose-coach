...
interface BurpeesTrackerProps {
  onExerciseComplete: (data: ExerciseData) => void;
  onBack: () => void;
}

export const BurpeesTracker: React.FC<BurpeesTrackerProps> = ({
  onExerciseComplete,
  onBack
}) => {
  ...
  const simulateExerciseTracking = () => {
    ...
    const interval = setInterval(() => {
      ...
      // Stop after 8-12 reps for demo (burpees are more intense)
      if (repCount >= 8 + Math.random() * 4) {
        clearInterval(interval);
        stopExercise();
      }
    }, 3000 + Math.random() * 1000); // Longer intervals for burpees
  };
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <p className="text-muted-foreground mb-4">
              High-intensity full-body exercise combining squat, plank, and jump. Start standing, squat down, jump back to plank, return to squat, then jump up.
            </p>
